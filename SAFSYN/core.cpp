...

                {
                    ...
                    if (modes & 1) {
                        if (abs_ls >= abs_s && abs_le <= abs_e && abs_ls < abs_le) {
                            r.loop_mode  = (modes & 2) ? LoopMode::PingPong : LoopMode::Forward;
...
static bool load_wav(const char* path, std::vector<int16_t>& out,
                     uint32_t& out_rate, uint8_t& out_ch)
{
    FILE* f = fopen(path, "rb");
    if (!f) return false;
    fseek(f, 0, SEEK_END);
...
static int sfz_note_to_midi(const std::string& s) {
    ...
    else if (i < s.size() && std::toupper((unsigned char)s[i]) == 'B') { semi--; i++; }
